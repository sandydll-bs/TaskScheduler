...
foreach ($t in $tasks) {
    try {
        ...
        $creationTime = $xml.Task.RegistrationInfo.Date
...
if ($userTasks.Count -gt 0) {
    foreach ($u in $userTasks) {
        ...
        if ($u.CreationTime) {
            try {
                $formattedDate = ([datetime]$u.CreationTime).ToString("dd/MM/yyyy HH:mm:ss")
                Write-Host "   CREATED: $formattedDate" -ForegroundColor Cyan
            }
            catch {
                Write-Host "   CREATED: $($u.CreationTime)" -ForegroundColor Cyan
            }
        }
        else {
            Write-Host "   CREATED: N/D" -ForegroundColor DarkGray
        }
