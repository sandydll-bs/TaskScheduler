...
foreach ($t in $tasks) {
    ...
    if ($isFlagged) {
        Write-Host "[!] $($t.TaskName) | LastRun: $($t.LastRunTime)" -ForegroundColor Red
    }
    else {
        Write-Host "$($t.TaskName) | LastRun: $($t.LastRunTime)"
    }
}
...
foreach ($t in (Get-ScheduledTask)) {
    ...
    try {
        ...
        if ($author) {
            ...
            if ($authorLower -match [regex]::Escape($currentUser)) {
...
if ($userTasks.Count -gt 0) {
    ...
    foreach ($u in $userTasks) {
        ...
        Write-Host "[USER] $($u.Task.TaskName) | LastRun: $($u.LastRunTime)" -ForegroundColor Yellow
